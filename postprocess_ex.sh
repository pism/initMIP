...
for exp in "ctrl" "asmb"; do
    ...
    ncks --64 -O -v mapping,thk,topg,usurf,climatic_mass_balance,bmelt,uvelsurf,vvelsurf,discharge_flux_cumulative,tempsurf,taub_mag,mask,run_stats,pism_config $infile $tmpfile1
    ncap2 -O -s '*sz_idt=time.size(); *discharge_flux[$time,$y,$x]= 0.f; *dHdt[$time,$y,$x]= 0.f; for(*idt=1 ; idt<sz_idt ; idt++) {discharge_flux(idt,:,:)=-(discharge_flux_cumulative(idt,:,:)-discharge_flux_cumulative(idt-1,:,:))/(time(idt)-time(idt-1))*1e12; dHdt(idt,:,:)=(thk(idt,:,:)-thk(idt-1,:,:))/(time(idt)-time(idt-1));} discharge_flux.ram_write(); dHdt.ram_write(); uvelsurf=uvelsurf/31556925.9747; vvelsurf=vvelsurf/31556925.9747; climatic_mass_balance=climatic_mass_balance/31556925.9747; sftgif[$time,$y,$x]=0b; where(mask==2 || mask==3) sftgif=1; sftflf[$time,$y,$x]=0b; where(mask==3) sftflf=1; sfrgrf[$time,$y,$x]=0b; where(mask==2) sfrgrf=1;' $tmpfile1 $tmpfile1
    ncatted -a units,discharge_flux,o,c,"kg s-1" -a units,dHdt,o,c,"m s-1" -a units,uvelsurf,o,c,"m s-1" -a units,vvelsurf,o,c,"m s-1" -a units,climatic_mass_balance,o,c,"kg m-2 s-1" $tmpfile1
    ...
    ncatted -a standard_name,acabf,o,c,"land_ice_surface_specific_mass_balance_flux"  -a standard_name,dHdt,o,c,"tendency_of_land_ice_thickness" -a standard_name,libmassbf,o,c,"land_ice_basal_specific_mass_balance_flux" -a standard_name,licalvf,o,c," 	land_ice_specific_mass_flux_due_to_calving" -a standard_name,sftgif,o,c,"land_ice_area_fraction" -a standard_name,sfrgrf,o,c,"grounded_ice_sheet_area_fraction" -a standard_name,sftflf,o,c,"floating_ice_sheet_area_fraction" -a standard_name,strbasemag,o,c,"magnitude_of_land_ice_basal_drag" -a standard_name,litempsnic,o,c,"temperature_at_ground_level_in_snow_or_firn" $tmpfile2
